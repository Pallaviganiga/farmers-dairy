        ...
        <div class="row">
            <div class="col-sm-12">
                        ...
                        <form class="needs-validation" action="#" method="post" >
                            <div class="form-row">
                                <div class="col-md-4">
                                    ...
                                    <select class="form-control" name="uno" pattern ="[A-Z]{1}[0-9]{3}" required />
								...
								?>
									<option value="<?php echo $userno; ?>"><?php echo $userno; ?>-<?php echo $fname; ?>&nbsp;<?php echo $lname; ?></option>
									<?php
									}
								}
								?>
							...
							if(isset($_POST['search']))
							{
								...
								if($num1>0)
								{
									while($row1=mysqli_fetch_array($result1))
									{
										...
										$_SESSION['phoneno']=$row1['phone_no'];
										$_SESSION['email']=$row1['email'];
										$_SESSION['accno']=$row1['accno'];
										$_SESSION['ifsc_code']=$row1['ifsc_code'];
								?>
                            <div class="col-md-6">
                                    <p for="validationCustom" style="text-align:right;font-weight:900">User Name: <?php echo $_SESSION['fname'];?>&nbsp;<?php echo $_SESSION['lname']; ?>
									<br>User Number:<?php echo $_SESSION['userno']; ?> 
									<br>User Phone:<?php echo $_SESSION['phoneno']; ?>
									<br>Email: <?php echo $_SESSION['email']; ?>
									<br>Account No: <?php echo $_SESSION['accno']; ?>
									<br>IFSC: <?php echo $_SESSION['ifsc_code']; ?></p>
				...
				?>
							...
							<div class="form-row">
							    <div class="form-group col-md-2">
                                    <label for="validationCustom">Date</label>
                                    <input type="date" class="form-control" value="<?php print(date("Y-m-d"));?>" name="date" required />
                                </div>
								<div class="form-group col-md-2">
                                    <label for="inputState">CLR in %</label>
                                    <select  class="form-control" id="inputState" name="clr" >
								...
								<div class="form-group col-md-2">
                                    <label for="inputEmail4">FAT in %</label>
                                    <select class="form-control" name="fat">
									<?php
								include 'config.php';
								$sql1="select * from add_rate group by fat";
								$result1=mysqli_query($con,$sql1);
								$num1=mysqli_num_rows($result1);
								$sI=0;
								if($num1>0)
								{
									while($row1=mysqli_fetch_array($result1))
									{
										...
										$fat=$row1['fat'];
										?>
									<option value="<?php echo $fat; ?>"><?php echo $fat; ?></option>
								<?php } } ?>
									</select>                              
								</div>
								<div class="form-group col-md-2">
                                    <label for="inputEmail4">SNF in %</label>
                                    <select class="form-control" name="snf">
											<?php
								$sql2="select * from add_rate where fat='$fat'";
								...
								if($num2>0)
								{
									while($row2=mysqli_fetch_array($result2))
									{
										$snf=$row2['snf'];
									?>
									<option value="<?php echo $snf; ?>"><?php echo $snf ;?></option>
								<?php } } ?>
									</select>
                                </div>
								...
								<div class="form-group col-md-2">
                                    ...
                                    <input type="number" class="form-control" min="0.1" name="liter" value="0" title="please enter a valid number" required="true" step=".01">
                                </div>
                            </div>
							<input type="text" value="<?php echo $fname; ?><?php echo $lname; ?>" name="username" hidden="true" required />
							<div class="form-row">
							<div class="form-group col-md-2">
...
if(isset($_POST['add']))
{
	...
	include("config.php");
		...
		$liter=$_POST['liter'];
		$clr=$_POST['clr'];
								$sql2="select amount from add_rate where fat='".$fat."' and snf='".$snf."'";
								$result2=mysqli_query($con,$sql2);
								$num2=mysqli_num_rows($result2);
								if($num2>0)
								{
		...
		$totalamt=$amount*$liter;
		$fullname=$_SESSION['fname'].' '.$_SESSION['lname'];
		$query = "insert into add_milk(name,user_no,phno,date,time,clr,fat,snf,liter,amount,totalamt,dippoid) values('".$fullname."','".$_SESSION['userno']."','".$_SESSION['phoneno']."','".$date."','".$time."','".$clr."','".$fat."','".$snf."','".$liter."','".$amount."','".$totalamt."','".$dippo."')";
                            ...
                            <table class="table table-hover">
								...
								?>
                                    <tr>
									 ...
									 <td colspan="8">
									 <div class="form-group col-md-2">
							<a href="printbill.php?id=<?php echo $am_id; ?>"> 
